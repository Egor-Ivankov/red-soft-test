...
function getParentElements(data: DataNode[]): { key: string; name: string }[] {
    const parentElements: { key: string; name: string }[] = [];

    data.forEach((item) => {
        parentElements.push({ key: item.key, name: item.name });

        if (item.children && item.children.length > 0) {
            const childParents = getParentElements(item.children);
            parentElements.push(...childParents);
        }
    });
    return parentElements;
}
export const parentElements: { key: string; name: string }[] = getParentElements(data);